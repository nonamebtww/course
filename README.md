...
- `course/`
  ...
  - `course.sln` / `*.vcxproj`  — файлы Visual Studio (если присутствуют)
  - `README.md`         — этот файл
...
Ввод:
- Выберите действие: `4`
- Точка x (нижний предел): `1`
- Верхний предел (для eps): `1.1`
...
- Все функции, возвращающие `MapEntry*`, выделяют память через `malloc`. Обязательная обязанность вызывающего — `free()` результата после использования.
- При ошибках (некорректный ввод, переполнение, невозможность вычисления) функции возвращают `NULL` (или `NAN` для `f`).

---

## Сборка и запуск (Visual Studio и CMake)

В проекте поддерживаются два способа сборки и запуска:

A) Windws - Visual Studio
...
2. Универсальный сценарий (работает с любым генератором по умолчанию):

```bash
# Unix-подобные системы или если используете Make/Ninja генератор
cmake -S . -B build
cmake --build build --config Release
# Запуск (пример для Unix): ./build/course
```
